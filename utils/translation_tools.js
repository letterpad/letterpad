...
switch (operation) {
    ...
    case "--add":
        if (
            incorrectParamsAlert(
                value,
                "Error: Provide a key value pair, key=value"
            )
        ) {
            return;
        }
        files.forEach(file => {
            ...
            if (key in contents.translations) {
                console.log(
                    chalk.yellow(`Key ${key} found in ${file}. File ignored.`)
                );
                return;
            }
            let changed = false;
    ...
    case "--del":
        files.forEach(file => {
            ...
            if (changed) {
                ...
                fs.writeFileSync(filePath, JSON.stringify(contents, null, 4));
                console.log(
                    chalk.green(`=> ✓ File written successfully: ${file}`)
                );
            } else {
                console.log(chalk.white(`Nothing changed in: ${file}`));
            }
    ...
    case "--set":
        ...
        files.forEach(file => {
            ...
            if (currentAction && reservedActions.indexOf(`-${name}`) >= 0) {
                if (`-${name}` === currentAction) {
                    console.log(
                        chalk.white(`Setting ${key} to ${value} in ${file}`)
                    );
                    contents.translations[key] = value;
                    change = true;
                }
