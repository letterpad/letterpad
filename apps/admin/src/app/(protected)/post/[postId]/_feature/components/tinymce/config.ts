...
export const blogEditorConfig = ({
  ...
  quickbars_selection_toolbar: "h1 h2 mark bold italic underline link",
  ...
  ai_shortcuts: [
    {
      title: "Tidy up",
      prompt:
        "Format this content, remove empty paragraphs, add headings for paragraphs if there are no headings, extra spaces, alignment, indentation, heading hirerchy, spelling, grammar, consistency without changing the meaning or losing any key information.",
      selection: true,
    },
    {
      ...
      onAction: function (_) {
        editor.windowManager.open({
          ...
          onSubmit(api) {
            ...
            //data.title
            editor.execCommand("mceAiDialog", true, {
              prompt: `Generate an outline for the post - ${data.title}. Ensure each headlines and its points flows logically and contributes to the overall flow of the post.`,
              generate: true,
              display: true,
            });
            api.close();
